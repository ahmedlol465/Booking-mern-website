...
test("should edit hotel", async ({ page }) => {
  ...
  // await expect(page.locator('[name="name"]')).toHaveValue("Dublin GetWay Updated")
  await page.locator('[name="name"]').fill("Dublin GetWay Updated")
  await page.getByRole("button", { name: "Save"}).click()
  await expect(page.getByText("Hotel Saved!")).toBeVisible()

  await page.reload();
  await expect(page.locator('[name="name"]')).toHaveValue("Dublin GetWay Updated")
  await page.locator('[name="name"]').fill("Dublin GetWay")
  await page.getByRole("button", { name: "Save"}).click()
  await expect(page.getByText("Hotel Saved!")).toBeVisible()
})